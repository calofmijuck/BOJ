...
ll accumulator[MAX_N], node_depth[MAX_N];
...
void multiply(ll* a, int size_a, ll* b, int size_b, vector<ll>& res) {
    vector<base> fa(a, a + size_a), fb(b, b + size_b);
    int n = 1;
...
void solve(int v) {
    ...
    sort(tree[centroid].begin(), tree[centroid].end(), [&](int a, int b) { return subtree_size[a] < subtree_size[b]; });
    ...
    memset(accumulator, 0, sizeof(ll) * (max_previous_depth + 1));
